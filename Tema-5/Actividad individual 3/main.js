...
function selector(split_text)
{
	...
	let resultA =[];
	let resultD =[];
	let wordsWithN = "Palabras con n: ";
	let wordsWithS = "Palabras con s: ";
	let wordsWithA = "Palabras con a: ";
	let wordsWithD = "Palabras con d: ";
	let n, s, d, a;
	...
	for( word of words )
	{
		...
		{
			resultN.push(word);
			n = wordsWithN + resultN;
		} 

		if (word[0] == "s")
		{
			resultS.push(word);
			s = wordsWithS + resultS;
		}

		if (word[0] == "a")
		{
			resultA.push(word);
			a = wordsWithA + resultA;
		}

		if (word[0] == "d")
		{
			resultD.push(word);
			d = wordsWithD + resultD;
		}

	};

	let result = n + "<br>" + s + "<br>" + a + "<br>" + d; 
	return result;
}
